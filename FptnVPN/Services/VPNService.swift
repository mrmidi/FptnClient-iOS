...
final class VPNService: ObservableObject {
    ...
    private func tunnelRuntimeOptions(settings: SettingsService) -> TunnelRuntimeOptions {
        ...
        // `dataPlaneMode` is already clamped for this build: flowProxy routes
        // every flow DIRECT and so exits from the device rather than an FPTN
        // server, and a value persisted by a debug build must never select it
        // in release. Hiding the UI is not sufficient — see
        // SettingsService.dataPlaneMode.
        return TunnelRuntimeOptions(
            ...
            dataPlaneMode: settings.dataPlaneMode
        )
    }
